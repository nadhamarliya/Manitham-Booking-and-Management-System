...
const PatientDetailDrawer = ({ isOpen, onClose, patient, getStatusBadgeStyles }) => {
  ...
  return (
    <>
      ...
      {/* FIXED: Swapped 'w-full max-w-xl' for 'w-full sm:max-w-xl' for mobile constraints */}
      <div className="fixed inset-y-0 right-0 w-full sm:max-w-xl bg-white shadow-2xl z-50 flex flex-col border-l border-slate-100 text-left animate-in slide-in-from-right duration-200">
        ...
        <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-900 text-white">
          <div className="overflow-hidden mr-2">
            <h2 className="text-xl font-bold tracking-tight truncate">{patient.name}</h2>
            <p className="text-xs text-slate-400 font-medium mt-0.5 truncate">Comprehensive Health & Registration Profile</p>
          </div>
          <button type="button" onClick={onClose} className="p-1.5 text-slate-400 hover:text-white rounded-xl hover:bg-white/10 transition-all cursor-pointer shrink-0">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 bg-slate-50/50">
          ...
          <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200/60 shadow-xs space-y-4">
            ...
            {/* FIXED: grid-cols-1 on phone screens, grid-cols-2 on small tablet profiles up */}
          ...
          <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200/60 shadow-xs space-y-4">
            ...
            {/* FIXED: grid-cols-1 for fluid smartphone alignment */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 gap-x-4 text-sm">
              ...
              <div className="sm:col-span-2"><p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Alternative Contact</p><p className="mt-0.5 font-semibold text-slate-700 font-mono">{patient.altContact || 'N/A'}</p></div>
              <div className="sm:col-span-2"><p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Permanent Address Details</p><p className="mt-1 p-3 bg-slate-50 rounded-xl border border-slate-200 text-slate-600 font-medium leading-relaxed break-words">{patient.address || 'N/A'}</p></div>
            </div>
          </div>
          ...
          <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200/60 shadow-xs space-y-4">
            ...
            <div>
              ...
              <p className="mt-1.5 text-sm font-bold text-slate-800 bg-indigo-50/50 border border-indigo-100 rounded-xl px-4 py-2.5 w-fit max-w-full break-words">
          ...
          <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200/60 shadow-xs space-y-4">
            ...
            {/* FIXED: grid-cols-1 for stacked mobile tracking blocks */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 gap-x-4 text-sm">
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-3 pt-2">
              <div className="p-2 sm:p-3 bg-slate-50 border border-slate-200 rounded-xl text-center">
                ...
                <p className="text-base sm:text-lg font-black text-slate-800 mt-0.5">{patient.height || '-'}<span className="text-[10px] sm:text-xs font-semibold text-slate-400 ml-0.5">cm</span></p>
              </div>
              <div className="p-2 sm:p-3 bg-slate-50 border border-slate-200 rounded-xl text-center">
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Weight</p>
                <p className="text-base sm:text-lg font-black text-slate-800 mt-0.5">{patient.weight || '-'}<span className="text-[10px] sm:text-xs font-semibold text-slate-400 ml-0.5">kg</span></p>
              </div>
              <div className="p-2 sm:p-3 bg-indigo-900 text-white rounded-xl text-center shadow-xs flex flex-col justify-center">
                <p className="text-[10px] text-slate-300 font-bold uppercase tracking-wider leading-none">BMI</p>
                <p className="text-base sm:text-lg font-black mt-0.5">{patient.bmi || 'N/A'}</p>
              </div>
            </div>
          </div>
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50 flex items-center justify-end">
          <button type="button" onClick={onClose} className="w-full sm:w-auto px-5 py-2.5 bg-slate-900 hover:bg-slate-800 text-sm font-semibold text-white rounded-xl shadow-sm cursor-pointer transition-all text-center">
            Close Profile
          </button>
        </div>
